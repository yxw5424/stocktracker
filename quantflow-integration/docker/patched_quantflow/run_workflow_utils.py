...
# [patched] 回测区间上限:原平台硬编码 3 年(SaaS 省算力,非技术必需)。
# 本地单机自跑无此顾虑,放宽为可配(默认 12 年),仍保留兜底防手滑打错年份。
import os as _os
_MAX_BACKTEST_DAYS = int(_os.getenv('MAX_BACKTEST_YEARS', '12')) * 365
        ...
        for node_id in layer:
            ...
            try:
                ...
                if hasattr(node_input, "start_date") and hasattr(node_input, "end_date"):
                    ...
                    if time_range > _MAX_BACKTEST_DAYS:
                        error_msg = "开始时间和结束时间之间的时间范围超过了上限(默认12年,可用 MAX_BACKTEST_YEARS 调整)"
                ...
                if hasattr(node_input, "test_start_date") and hasattr(node_input, "test_end_date"):
                    ...
                    print(f"时间范围：{time_range}天")
                    if time_range > _MAX_BACKTEST_DAYS:
                        error_msg = "回测开始时间和回测结束时间之间的时间范围超过了上限(默认12年,可用 MAX_BACKTEST_YEARS 调整)"
                        logger.error(error_msg)
                        raise Exception(error_msg)
                ...
                if hasattr(node_input, "predict_start_date") and hasattr(node_input, "predict_end_date"):
                    ...
                    print(f"时间范围：{time_range}天")
                    if time_range > _MAX_BACKTEST_DAYS:
                        error_msg = "预测开始时间和预测结束时间之间的时间范围超过了上限(默认12年,可用 MAX_BACKTEST_YEARS 调整)"
                        logger.error(error_msg)
                        raise Exception(error_msg)
